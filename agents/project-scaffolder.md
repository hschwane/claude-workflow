...
- **Format what you wrote first.** `ci.sh fast` runs the project's format check over files you have just authored by hand; run the project's formatter (`npm run format`, `uv run ruff format .`, `cargo fmt`) before verifying, or the first gate run is red for a reason that has nothing to do with the scaffold.
