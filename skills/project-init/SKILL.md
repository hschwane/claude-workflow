...
**Reading is all this step decides.** The scaffolder installs the *entire* library into `.claude/guidelines/` regardless of what matched here — matching governs what shapes the design, not what lands on disk. The one thing to pass on is any **user-global** guideline from `~/.claude/guidelines/` that matched: those are not in the library, and `~/.claude/` is ephemeral in cloud sessions, so hand their absolute paths to the scaffolder as `GLOBAL_GUIDELINES` or they are lost.
...
3. **Branching model**: main-only / Git Flow. The release branch is `main` either way — git-flow adds `develop`, it does not rename `main`. State the real difference when you ask, because it is what the choice is actually about:
   - **main-only** — every landing on the trunk *is* a release. `/pr` has nothing to target and hands to `/release`. Simple; right when git-flow would be overkill. Several tickets can share a branch and release together.
   - **git-flow** — landing and releasing are separate events. Features merge into `develop`, `/release` merges `develop` → `main` and tags, so `main`'s tip always equals the latest release.

3b. **Reference environment** — ask **only when the answer to 3 is git-flow**: "Should `develop` have its own deployed environment, updated after every merge, so you can try the merged state before a release? [yes (recommended for anything with a UI) / no]". This is the `REFERENCE_ENV` value the scaffolder needs, and it decides whether `scripts/deploy-reference.sh` and `reference-deploy.yml` are installed at all — do not guess it. `yes` also adds a tech-backbone ticket for standing the environment up; the scaffolder does not create it, because it needs real platform credentials and its own database. Under main-only the answer is always `no`.
...
- **tech-backbone (5–10 items):** Build system working, CI green (lint/type-check/test), core infrastructure provisioned (database, auth provider, cloud services — specific to the project type and deploy target from steps 3–5), release/deploy pipeline end-to-end, smoke test / health check endpoint so the user can verify the skeleton is alive in the deployed environment.
  - **A version the healthcheck can assert.** `scripts/healthcheck.sh` refuses to confirm a release unless a probe compares the live version against the expected one, so until something exposes a version, `/release` stops. Ticket it here.
  - **An exercised rollback.** It is the only safety net once a merge can ship, and an untested rollback is not a safety net. Run it once against the real platform and write the procedure into `docs/dev/deploy.md`.
  - **The reference environment**, when `REFERENCE_ENV` is `yes` — a separate service with its own database, secrets and domain, following `develop`. It needs real credentials, so it is a ticket, not scaffolding.
