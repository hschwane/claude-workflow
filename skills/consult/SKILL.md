---
name: consult
description: Consult the top-tier model (the "advisor") on a hard decision while keeping the session on its normal model. Use PROACTIVELY when stuck (two failed attempts at the same problem), facing an architecture- or security-relevant decision, or genuinely unsure between approaches. Runs inline with the full session context; cost is one elevated turn.
...
You are now on the top tier (best available model, high effort) for the rest of this turn, with the full session context. This is a consultation, not a work session — no implementation on this tier.
...
4. **Hand back.** The elevated tier applies only to this turn — it reverts automatically on the next turn. Continue the interrupted work on the normal session model.
