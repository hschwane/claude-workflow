...
You own the outcome. Apply the advice (or decide against it — it's advice). **If it resolved an actual decision**, append a dated entry to `.claude/memory/decisions.md` (question → decision → why), and if it shapes architecture write the reasoning into `docs/dev/architecture.md` with the one-line record pointing at it. For pure ideation or a debugging pointer that didn't settle anything yet, no record is needed — just continue the work with the new angle. If the advisor said the call needs information only the user has, surface that as a `[USER]` question or a `## Blocked` note rather than guessing.
