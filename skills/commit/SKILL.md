...
### 5. Push
Feature/fix branches: push (`git push -u origin {branch}`) — cheap backup, the gate already ran. Direct commits to `develop`/`main`/`master` are fine for spec-only changes and for the merges `/pr` and `/release` perform — and those integration-branch pushes are authorized by the invoked skill, not something to re-ask about (see **Merging** in `CLAUDE.md`); otherwise work on a branch.
