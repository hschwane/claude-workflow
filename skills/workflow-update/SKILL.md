...
- **The topic files are selective, and each is diffed before it is replaced.** Refresh the ones the project *has* — but **diff each against OLD first**. A project that appended its own section to a shipped guideline (a region override, a tightened limit) loses it silently otherwise, and "upstream fixed a guideline this project had tweaked" is the most ordinary event there is. Show the edit and ask where it goes: **[a]** a dated deviation in `.claude/memory/decisions.md` naming the guideline, for a *rule*; **[b]** `.claude/memory/gotchas.md`, for a *fact that cost someone time*; **[c]** discard. Never offer "leave it in the guideline" — this directory is plugin-owned, so the next update deletes it again. Only then overwrite. `README.md` in that directory has the same exposure. Never bulk-install the library — a project with twelve unmatched guidelines has an index `/plan` stops trusting. Offer **every** guideline in the new `templates/guidelines/` that the project does not have and whose `LIBRARY.md` "install when" matches it — *not* only the ones new since the installed version. The usual case is a guideline that has shipped for years and only now matches, because the project grew a background job or a UI; filter on "newly shipped" and that one is never offered on any update, ever. This is a step, not a sentiment.
...
**A `mixed` file can carry install-time tokens too, and the scripts-only grep above will not catch them.** `.github/workflows/ci-*.yml` ships `branches: {{CI_BRANCHES}}` as its push trigger. Substitute the **whole bracketed list** from the `workflow-settings` block before writing: `[<trunk-branch>]` under `branching: main-only`, `[<trunk-branch>, develop]` under `git-flow`. A literal `{{CI_BRANCHES}}` there is not an error anyone sees — the workflow parses, commits, and then matches no branch ever again — and dropping `develop` under git-flow kills CI on the branch every feature merges into. Silent CI death is worse than exit 127. Grep the workflow files you wrote for `{{[A-Z_]` and treat a hit exactly like one in a script.
...
| `trunk-branch` | no v2 equivalent — v2 hardcoded `main`/`master` in every workflow | `git branch --show-current`, or the release branch `lifecycle.md` names (git-flow's "merges `develop → X`" makes X the trunk). **Never the template default.** |
...
Anything still unresolved: **ask, do not guess.** Defaults if the user has no opinion: `testing-scope: unit+integration`, `branching: main-only`, `github: yes`, `ci-on-claude: no`, `release-runner: local`, `deploy: none`, `version-source` from whichever manifest the project has, else `none`, and `trunk-branch` from **the repo's own current branch**.

**`trunk-branch` never falls through to the template default.** It is the one setting whose wrong value is silently destructive: §5a substitutes it into the CI workflow's push trigger and `/release` merges and tags on it, so writing `main` into a repo whose trunk is `master` gives you CI that never fires again and a release that targets a branch which does not exist. Read it from the repo (`git branch --show-current`, or the branch `lifecycle.md` names as the release target) — this is the same rule `/workflow-settings` states for that row: *never assume `main`*.
...
- **No `{{…}}` token that *this update introduced*** remains anywhere. Take the set from `git diff --name-only` against the pre-update HEAD rather than from a list, then apply two exclusions and one rule:
  ...
  - **Inside `.github/workflows/`, `${{ … }}` is GitHub Actions expression syntax, never a template token.** `group: ci-${{ github.ref }}` and `${{ secrets.NPM_TOKEN }}` are correct code. Match `{{[A-Z_]` rather than `{{` there, so the real one — `{{CI_BRANCHES}}` in the push trigger — still stops you. A check that fires on every workflow file teaches the operator to wave workflow hits through, which is exactly how a dead CI trigger ships.
...
- The `workflow-settings` block has every key the new version ships and no stale ones, **and every value is one the target version's `/workflow-settings` table lists for that key** — for `version-source`, that the named file exists; for `trunk-branch`, that `git rev-parse --verify` resolves it. Checking keys alone passes a block whose values are template defaults, which is how a wrong `trunk-branch` reaches a commit. This block is always-loaded and `/workflow-settings` will reject a bad value the first time anyone edits it.
