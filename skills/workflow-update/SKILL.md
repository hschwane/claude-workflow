...
- **"Unchanged upstream" only excuses a path the project already has.** A delivered path the project is *missing* goes on the work list whatever the diff says. `templates/hooks/hooks.json` is often byte-identical across versions — drop it here and `.claude/settings.json` is never installed, leaving every refreshed hook present but unwired, and nothing later notices.
  **This applies to `plugin` paths and to the `mixed` ones the workflow cannot run without** (`.claude/settings.json`, `scripts/ci.sh`, `scripts/release.sh`, `scripts/healthcheck.sh`, `scripts/dev.sh`, `docs/dev/README.md`) — plus the two `plugin` scripts `scripts/gate-status.sh` and `scripts/criteria-check.sh`, which carry no project content and are simply installed. **`scripts/deploy-reference.sh` and `.github/workflows/reference-deploy.yml` are the exception: absent is the normal state.** They belong only to a `git-flow` project that chose a reference environment; report them missing, never create them, and never create one without the other. A missing **`project`**-class path is not a gap — a TypeScript repo has no `Cargo.toml`, and a project that never released has no `docs/changelog/`. Leave those off the work list entirely rather than reporting nine other-language manifests on every update forever — **except the handful §5a deliberately creates** because the `CLAUDE.md` and `CONTRIBUTING.md` this update installs index them (`docs/dev/setup.md`, `docs/VISION.md`, `docs/specs/*`, `docs/user/README.md`, `.env.example`). Those belong on the work list as *to create*, not as missing manifests; read §5a's `project` section before you finalise the list, or a fresh always-loaded index will point at four paths that do not exist.
...
The manifest's `mixed` entries — today `scripts/ci.sh`, `scripts/release.sh`, `scripts/healthcheck.sh`, `scripts/dev.sh`, `scripts/deploy-reference.sh`, `docs/dev/README.md`, the two workflow YAMLs, `.github/dependabot.yml` and `.claude/settings.json`.

**`docs/dev/README.md` merges by rows, not by file.** Its four scaffolded rows are the plugin's; every other row is a document the project wrote and must survive. Replace the header and the four known rows, keep the rest. Losing a row does not break anything visibly — it just makes that document invisible to `/plan` and to `code-explorer`, which is worse than a build error because nothing reports it. Read the class off the manifest rather than this list, which ages. Markers cannot work here: the plugin's scaffolding and the project's content are interleaved, or the format carries no comments.
...
**Two more token carriers, both new in 3.1.0.** `.github/workflows/release.yml` installed from `release-github.yml` ships `{{TOOLCHAIN_STEPS}}` — fill it with the same setup and install steps as `ci.yml` (the npm and pypi variants carry their own and have no token). `scripts/healthcheck.sh` ships `{{HEALTHCHECK_PRODUCTION}}` and `{{HEALTHCHECK_REFERENCE}}`; fill production from `docs/dev/deploy.md`, and **delete the whole `reference)` branch** where the project has no reference environment, rather than leaving a probe-less branch that verifies nothing.
