...
**Check preferences and project notes:** read the indexes — `.claude/preferences/INDEX.md` (workflow preferences), `.claude/project-notes/INDEX.md` (this project's own standing rules), and if present the user-global `~/.claude/preferences/INDEX.md`. For every trigger that matches a ticket's technology/feature, read that file.
