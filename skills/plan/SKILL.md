...
- **Acceptance criteria** — **observable** statements (an action → an expected, checkable result: "run `x --foo` → prints Z"; "POST /bar → 200 + `{id}`"; "click Save → row persists + toast"). These are the contract `/verify` checks against, so they must be demonstrable, not vague ("should work").
  ...
  **Two anchors that constrain each other beat one citation.** A single cited value is one line of prose that nothing downstream re-derives: write `JD 2451544.5 = 2000-01-01 12:00 UT (J2000.0)` — a wrong number with a real-sounding citation — and the code, the tests, `/verify`'s table and a fresh reviewer all agree with it. Two independent published pairs cannot both hold under a wrong constant, so a mistake in either shows up as a contradiction rather than as consensus. Where an authority publishes more than one datum, take two.
