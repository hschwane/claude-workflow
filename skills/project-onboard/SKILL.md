...
- **The healthcheck needs somewhere the released version is readable.** `scripts/healthcheck.sh` refuses to confirm a version unless a probe compares against it. **Work out where that is before concluding anything is blocked** — the answer differs by release type, and only one of the three cases is a blocker:
  - **Publishes to a registry** (npm/pypi/crates): the version is already readable — `npm view <pkg> version`, the PyPI/crates.io JSON API. Nothing is missing, so no ticket. The hazard here is a different one: the registry answers for *whoever owns the name*, so a probe against a name that is not yours reports a confident green about a stranger's package. Compare provenance too (`repository.url` against this repo) and make sure the manifest carries the field that makes that possible.
  - **Ships a binary or a CLI**: `./dist/app --version`, or whatever the built artifact prints. Also readable, also no ticket — but check the flag actually exists rather than assuming it.
  - **A deployed service that reports no version at runtime**: *this* is the blocking case, and it is common. Don't weaken it to a bare liveness probe: write the `version_probe` form against the endpoint it will have, add a blocking backlog ticket for version visibility, and record it in `tech-debt.md`. `/release` will stop until that ships — deliberately, and the user should hear it in the step 6 report rather than discover it at their first release.
...
- **A lockfile.** Generate and commit one if the project has none. Do not condition this on what CI currently demands: `uv sync` *writes* `uv.lock` as a side effect, so step 5's `git add -A` commits it either way — better deliberately, and named in the report, than silently.
  **Unless the project's `.gitignore` already excludes it** — which for a Rust library is the cargo-recommended default, not an oversight. You cannot resolve that by editing `.gitignore`: it is on the never-touch list, and `git add -A` will not add an ignored file anyway, so "generated and committed" would be a line in your report that did not happen. Respect the project's choice, and make the *gate* consistent with it instead: no `--locked` / `npm ci` install step, because those fail outright without a lockfile. Record the pairing in `decisions.md` and offer the alternative once — commit the lockfile and drop the ignore rule — as the user's call, not yours.
...
- **`scripts/dev.sh`** — the project's existing dev-run command plus any migration and seed steps a test instance needs. `{{DEV_INFO}}` must be enough to drive the thing blind: the exact URL or command and any test credentials, because the smoke-tester gets nothing else — normally several lines, written as real newlines inside the single quotes, never as `\n` (it is printed with `printf '%s\n'` and would ship as two literal characters). Delete `{{DEV_RUN}}` for a library or CLI — there is nothing to keep running.
...
**If there was no CI at all, it is already installed — do not install it again.** The scaffolder's rule is "skip what the project already provides", so with nothing there it copied `templates/github/ci-{language}.yml` to `.github/workflows/ci.yml` back in 3a, `{{CI_BRANCHES}}` and all. There is nothing to offer here, and offering it contradicts the standing decision that the workflow is always installed and always kept current. What is left for you is one verification: `grep -n 'CI_BRANCHES\|branches:' .github/workflows/ci.yml` and confirm the token — which appears **twice**, once on `push` and once on `pull_request` — is gone from both and that the branch list is this repo's real trunk — `[master]` for a `main-only` repo whose trunk is `master`, `[master, develop]` under git-flow. An unsubstituted token, or `main` assumed on a `master` repo, is valid YAML and a dead trigger that reports nothing. Fix it in place if it is wrong, and note the fix in the report.
