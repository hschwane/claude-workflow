...
| `/refine FEAT-001` | Depth scales with a complexity triage: **trivial** specs get one combined `tech-planner` fast-track pass (escalates itself if it finds hidden complexity); **small/medium/large** run the full `requirements-engineer` + `tech-planner` iteration (max 1 / 1 / 3 rounds) until the spec meets the Definition of Ready — on the tier the size dictates (sonnet-high → opus-high → best-high). Writes the per-ticket `routing:` block. Bug tickets pull platform logs (e.g. Railway) as evidence first. Pass **multiple IDs** to batch: questions up front, then all tickets complete autonomously |
