...
Two orthogonal dials tune each agent: **model** (`model` frontmatter — the capability tier) and **effort** (`effort` frontmatter — how hard to think within that tier). Each agent pins the cheapest model that reliably does its job; only the judgment-heavy agents follow your session model. This mirrors the established pattern — plan/review with the strongest model, execute with Sonnet, extract/digest with Haiku (the built-in Explore agent runs on Haiku for the same reason):
...
**Effort routing** (`effort` frontmatter, `low`/`medium`/`high`/`xhigh`/`max`): the default is `high` on current models, so an unpinned agent already thinks hard — which is wasteful for mechanical work. The purely mechanical/fact-gathering agents therefore pin `effort: low` (`code-explorer`, `test-runner`, `workflow-coach`, `project-scaffolder`): they gather, run, condense, or copy — deep reasoning buys nothing there, so low effort cuts latency and tokens with no quality loss. Judgment agents are left unpinned so their effort **scales with your session** (`inherit`-model reviewers rise to `max` when you do); pinning effort would both floor *and* cap them, breaking that upward scaling. Precedence: `CLAUDE_CODE_EFFORT_LEVEL` env var > frontmatter `effort` > session level.
